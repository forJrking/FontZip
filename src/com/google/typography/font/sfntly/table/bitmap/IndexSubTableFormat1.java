...
{
  private IndexSubTableFormat1(ReadableFontData paramReadableFontData, int paramInt1, int paramInt2)
  {
  ...
  {
    return imageDataOffset() + this.data.readULongAsInt(EblcTable.Offset.indexSubTable1_offsetArray.offset + paramInt * FontData.DataSize.ULONG.size());
  }
    ...
    extends IndexSubTable.Builder<IndexSubTableFormat1>
  {
    ...
    {
      return EblcTable.Offset.indexSubHeaderLength.offset + (paramInt3 - paramInt2 + 1 + 1) * FontData.DataSize.ULONG.size();
    }
    ...
    private void initialize(ReadableFontData paramReadableFontData)
    {
      ...
      if (paramReadableFontData != null)
      {
        ...
        for (int j = 0; j < i; j++) {
          this.offsetArray.add(Integer.valueOf(paramReadableFontData.readULongAsInt(EblcTable.Offset.indexSubTable1_offsetArray.offset + j * FontData.DataSize.ULONG.size())));
        }
      }
    }
    ...
    protected IndexSubTableFormat1 subBuildTable(ReadableFontData paramReadableFontData)
    {
      return new IndexSubTableFormat1(paramReadableFontData, firstGlyphIndex(), lastGlyphIndex(), null);
    }
    ...
    protected int subSerialize(WritableFontData paramWritableFontData)
    {
      ...
      if (!modelChanged())
      {
        i += internalReadData().slice(EblcTable.Offset.indexSubTable1_offsetArray.offset).copyTo(paramWritableFontData.slice(EblcTable.Offset.indexSubTable1_offsetArray.offset));
      }
      else
      {
        ...
        while (localIterator.hasNext())
        {
          ...
          i += paramWritableFontData.writeULong(i, localInteger.intValue());
        }
      }
      return i;
    }
    ...
    {
      private int glyphId = IndexSubTableFormat1.Builder.this.firstGlyphIndex();
      
      public BitmapGlyphInfoIterator() {}
      
      public boolean hasNext()
      {
        return this.glyphId <= IndexSubTableFormat1.Builder.this.lastGlyphIndex();
      }
      ...
      public BitmapGlyphInfo next()
      {
        ...
        BitmapGlyphInfo localBitmapGlyphInfo = new BitmapGlyphInfo(this.glyphId, IndexSubTableFormat1.Builder.this.imageDataOffset(), IndexSubTableFormat1.Builder.this.glyphStartOffset(this.glyphId), IndexSubTableFormat1.Builder.this.glyphLength(this.glyphId), IndexSubTableFormat1.Builder.this.imageFormat());
        this.glyphId += 1;
...
/* Location:              C:\Users\Ethan\Desktop\FontZip\FontTool\sfnttool.jar!\com\google\typography\font\sfntly\table\bitmap\IndexSubTableFormat1.class
